...
- `-DN` `-DiskNumber` - номер диска. Можно узнать командой `Get-Disk`. Но если не ввести номер диска в параметры скрипта при запуске, скрипт сам выведет информацию и спросит номер диска. Обязательный параметр.
- `-FS` `-FileSystem` - файловая система. Может принимать следующие значения:
...
- `-S` `-Sleep` - перерыв между командами (в секундах).
...
Обнулить диск `3` и форматировать в файловую систему `NTFS`:
